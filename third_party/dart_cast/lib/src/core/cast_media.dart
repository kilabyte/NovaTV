...
class CastMedia {
  ...
  /// NovaTV patch: when true, [ChromecastSession] loads [url] on the receiver
  /// AS-IS, bypassing dart_cast's built-in proxy / m3u8 rewriter. Use when the
  /// caller already serves a ready-to-play, CORS-enabled, header-injected HLS
  /// (e.g. NovaTV's on-device TS->HLS transmux server). The stock proxy rewrites
  /// live IPTV playlists in a way the receiver rejects (LOAD_FAILED).
